...
class DatabaseService {
  ...
  getLogs(filters: {
    ...
    const effectiveLimit = filters.query && this.isAdvancedQuery(filters.query) 
    ...
    if (effectiveLimit) {
      ...
      if (filters.offset && (!filters.query || !this.isAdvancedQuery(filters.query))) {
        sql += ' OFFSET ?';
  ...
  getLogCount(filters: {
    ...
    if (filters.query && this.isAdvancedQuery(filters.query)) {
      const logs = this.getLogs({ ...filters, limit: undefined, offset: undefined });
      return logs.length;
    }
