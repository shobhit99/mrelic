...
const SERVICES = ['api-gateway', 'user-service', 'payment-service', 'inventory-service', 'notification-service'];
...
function generateRandomLog() {
  ...
  switch (service) {
    case 'api-gateway':
      const endpoints = ['/api/users', '/api/products', '/api/orders', '/api/payments'];
    ...
    case 'user-service':
      const userActions = ['login', 'logout', 'register', 'update_profile', 'reset_password'];
    ...
    case 'inventory-service':
      const actions = ['item_added', 'item_removed', 'stock_updated', 'inventory_check'];
      const items = ['laptop', 'smartphone', 'headphones', 'monitor', 'keyboard'];
      const quantities = [1, 2, 5, 10, 20, 50];
      
      const inventoryAction = actions[Math.floor(Math.random() * actions.length)];
    ...
    case 'notification-service':
      ...
      const notificationStatus = notificationStatuses[Math.floor(Math.random() * notificationStatuses.length)];
...
async function sendLog() {
  ...
  try {
    ...
    console.log(`Log sent: ${log.service} - ${log.level} - ${log.message.substring(0, 50)}...`);
...
console.log(`Starting to send logs to ${API_ENDPOINT} every ${LOG_INTERVAL_MS}ms...`);
console.log('Press Ctrl+C to stop');

// Send an initial batch of 10 logs
Promise.all(Array(10).fill().map(() => sendLog()))
  .then(() => {
